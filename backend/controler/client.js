...
module.exports = {
              ...
              admin
                ...
                .then((customToken) => {
                  const response = {
                    ...
                    user_id: results.insertId, // Assuming results.insertId returns the user_id generated by the SQL query
                  };
                  res.send(response);
                })
                .catch((error) => {
  ...
  signIn(req, res) {
    ...
    client.getOneClientByEmail(email, (error, user) => {
      if (error) {
        ...
        res.status(500).send({ error: "Sign-in failed" });
        return;
      }
  ...
  updateClient(req, res) {
    const { username, phone_number, password, newpassword, email } = req.body;

    client.getOneClientByEmail(email, (error, user) => {
      if (error) {
        console.error("Error retrieving user:", error);
        res.status(500).send({ error: "No client found" });
        return;
      }

      if (!user) {
        console.error("Cannot update user. User not found.");
        res.status(401).send({ error: "Invalid email or password" });
        return;
      }
